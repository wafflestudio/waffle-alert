...
> 이번 단계는 **각 source가 `AlertEvent`를 채워서 NotifyService에 넘기면 Discord로 나간다**까지만 만든다.

## 1. 전체 흐름

```
[팀원 A] Alertmanager webhook   ──┐
[팀원 B] OCI Monitoring poller  ──┼──→  AlertEvent  ──→  NotifyService.notify(event)  ──→  Discord
[팀원 C] OCI Cost poller        ──┘                            (공통, 준병 담당)
```

- 팀원 3명은 **각자 소스의 payload/API 응답을 `AlertEvent`로 변환해서 `NotifyService.notify(event)` 한 줄만 호출**하면 끝.
...
## 3. NotificationPort / NotifyService 시그니처
...
둘 다 `DiscordNotificationAdapter` 내부 고정 매핑으로 처리. 팀원은 `source`/`team` 필드만 정확히 채우면 된다.

- **채널**: `event.source` → `discord.channel-ids`(yml)의 키로 매핑 (기존 `channelKeyOf(source)` 로직 재사용)
  - `ALERTMANAGER` → `prometheus-alert`
  - `OCI_COST` → `oci-cost`
  - `OCI_MONITORING` → `oci-monitoring`
- **멘션**: `event.team` 문자열 → `DiscordMentionRole` enum 매핑 테이블(어댑터 내부). 매핑 안 되는 team이면 멘션 생략(로그만 남김).
...
공통으로: 변환 후 `NotifyService.notify(event)` 호출 지점까지만 구현하면 각자 담당 끝.

## 6. 내(공통) 담당 작업 목록

- [ ] `AlertEvent` 필드를 위 20개 스펙대로 확정/수정
- [ ] `NotificationPort.notify(incident, event)` → `notify(event)` 시그니처 변경
- [ ] `DiscordNotificationAdapter`
  - [ ] `formatMessage`를 `AlertIncident` 대신 `AlertEvent` 기준으로 재작성
  - [ ] `channelKeyOf(source)` 유지 (그대로 재사용)
  - [ ] `team → DiscordMentionRole` 매핑 테이블 추가, 메시지 앞에 멘션 붙이기
- [ ] `DiscordTestController` — 새 시그니처에 맞춰 테스트 엔드포인트 정리 (AlertEvent를 직접 구성해서 보내는 형태로)
- [ ] 팀원들에게 이 문서 공유 + `AlertEvent` 필드/NotifyService 시그니처 확정 알림

## 7. 다음 단계 (보류 중, 지금 안 함)

- `AlertIncident`/`AlertEventLog` DB 저장, fingerprint 기준 묶기 (`IncidentService`)
- OCI evaluator (임계치 판단, RESOLVED 자동 판단)
- Slack 어댑터