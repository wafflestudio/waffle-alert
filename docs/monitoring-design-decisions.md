...
현재 등록된 rule은 OCI MySQL CPU, memory, DB volume이다. 일치하는 rule이 없으면 이벤트를 만들지 않는다.
...
| Metric 범위 | OCI MySQL CPU, memory, DB volume을 지원한다. | 필요한 순서대로 connection, backup failure adapter와 rule을 추가한다. |
